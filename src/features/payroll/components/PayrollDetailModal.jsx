...
const adjustSchema = z.object({
  manualBonus: z.coerce.number().min(0, 'Tidak boleh negatif').optional().or(z.literal('')),
  deductions:  z.coerce.number().min(0, 'Tidak boleh negatif').optional().or(z.literal('')),
  kasbon:      z.coerce.number().min(0, 'Tidak boleh negatif').optional().or(z.literal('')),
}).refine(
  (d) => d.manualBonus !== '' || d.deductions !== '' || d.kasbon !== '',
  { message: 'Harus mengisi setidaknya satu field' }
)
...
const PayrollDetailModal = ({ open, onClose, payrollId, canManage }) => {
  ...
  if (isLoading) {
    return (
      <Modal open={open} onClose={onClose} title="Detail Penggajian" size="md">
        <div className="flex items-center justify-center py-10 text-muted-foreground">
          <Loader2 className="w-5 h-5 animate-spin mr-2" />
          Memuat detail penggajian...
        </div>
      </Modal>
    )
  }
    ...
    return (
      <Modal open={open} onClose={onClose} title="Detail Penggajian" size="md">
        <div className="py-10 text-center text-sm text-destructive">
          {error?.response?.data?.message ?? 'Gagal memuat data penggajian.'}
        </div>
      </Modal>
    )
  }
  ...
  const handleApprove = () => {
    approveMutation.mutate(payroll._id, {
      onSuccess: () => { toast.success('Payroll disetujui') },
      onError:   (err) => toast.error('Approval gagal', err?.response?.data?.message),
    })
  }

  const handleReject = () => {
    rejectMutation.mutate(payroll._id, {
      onSuccess: () => { toast.success('Payroll dikembalikan ke draft') },
  ...
  const handleMarkPaid = () => {
    paidMutation.mutate(payroll._id, {
      onSuccess: () => { toast.success('Payroll ditandai sebagai lunas') },
      onError:   (err) => toast.error('Gagal', err?.response?.data?.message),
    })
  }
  ...
  const onAdjustSubmit = (data) => {
    ...
    adjustMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success('Payroll disesuaikan')
          setShowAdjust(false)
        },
        onError: (err) => toast.error('Penyesuaian gagal', err?.response?.data?.message),
      }
    )
  }
        ...
        <div className="flex-1 overflow-y-auto pr-1">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {payroll.salaryType === 'monthly' ? 'Bulanan' : 'Harian'} — {payroll.workingDays} hari kerja
              </p>
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { label: 'Hadir', value: payroll.presentDays, color: 'text-brand-600' },
                { label: 'Tidak Hadir',  value: payroll.absentDays,  color: 'text-destructive' },
                { label: 'Cup',    value: payroll.totalCupsSold, color: 'text-amber-600' },
          ...
          <div>
            ...
            <div className="bg-muted/30 rounded-lg px-3 py-1">
              <CalcRow label="Gaji Pokok"  value={formatIDR(payroll.salaryEarned)} />
              {payroll.payrollType === 'commission' && (
                <>
                  <CalcRow label="Pendapatan"         value={formatIDR(payroll.totalRevenue)} />
                  <CalcRow label="Persentase Komisi" value={`${payroll.commissionPercentage ?? 0}%`} />
                </>
              )}
              <CalcRow label="Komisi" value={`+ ${formatIDR(payroll.commission)}`}              positive={payroll.commission > 0} />
              <CalcRow label="Bonus Cup"     value={`+ ${formatIDR(payroll.cupsBonus)}`}    positive />
              <CalcRow label="Uang Makan" value={`+ ${formatIDR(payroll.mealAllowanceTotal)}`}       positive={payroll.mealAllowanceTotal > 0} />
              <CalcRow label="Bonus Tingkat Harian"        value={`+ ${formatIDR(payroll.dailyTierBonus)}`}          positive={payroll.dailyTierBonus > 0} />
              <CalcRow label="Bonus Kehadiran Mingguan" value={`+ ${formatIDR(payroll.weeklyAttendanceBonus)}`}   positive={payroll.weeklyAttendanceBonus > 0} />
              <CalcRow label="Bonus Manual"   value={`+ ${formatIDR(payroll.manualBonus)}`}  positive={payroll.manualBonus > 0} muted={!payroll.manualBonus} />
              <CalcRow label="Potongan"     value={`− ${formatIDR(payroll.deductions)}`}   negative={payroll.deductions > 0} muted={!payroll.deductions} />
              <CalcRow label="Kasbon"         value={`− ${formatIDR(payroll.kasbon)}`}       negative={payroll.kasbon > 0} muted={!payroll.kasbon} />
              <CalcRow label="Total Gaji"      value={formatIDR(payroll.totalPay)} bold />
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit(onAdjustSubmit)} noValidate>
              <div className="p-3 rounded-lg border border-border bg-muted/20 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <FormField label="Bonus Manual" error={errors.manualBonus?.message}>
                    <Input
                  ...
                  <FormField label="Potongan" error={errors.deductions?.message}>
                    <Input
                      {...register('deductions')}
                      type="number" min="0"
